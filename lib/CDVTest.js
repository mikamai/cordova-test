...
export default class CDVTest {
  ...
  buildAndroid() {
    const apkPath = path.join(this.platform_path, 'build/outputs/apk/', 'android-debug.apk');
    // build android
  ...
  runTest(test, list) {
    ...
    const jestParams = ['--runInBand'];
    let params = [test];
    const env = Object.assign({}, process.env, { PLATFORM: this.platform });
    if (this.env === 'jest') params = params.concat(jestParams);
    if (this.babel) params = params.concat(mochaBabelParams);
    if (this.teamcity) params = params.concat(mochaTeamCityParams);
    const self = this;
    this.logger.echo(`Running test; ${this.env} ${test}`);
    const child = childProcess.spawn(this.env, params, { stdio: 'inherit', env });
    child.on('close', () => {
